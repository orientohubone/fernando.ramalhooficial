      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pt-20 pb-40">
        <header className="mb-24 space-y-6">
          <div className="flex items-center gap-4">
            <div className="h-[2px] w-12 bg-[#58B573]"></div>
            <span className="text-[10px] font-black uppercase tracking-[0.6em] text-[#58B573]">Proprietary Intelligence</span>
          </div>
          ...
          <p className="text-xl md:text-2xl text-neutral-500 max-w-2xl font-medium tracking-tight">
            {t.description}
          </p>
        </header>

        {/* Filters */}
        <div className="flex flex-wrap gap-2 mb-16 border-b border-neutral-900 pb-8">
          {Object.entries(t.categories).map(([key, label]) => (
            <button
              key={key}
              onClick={() => setActiveFilter(key)}
              className={`px-4 py-2 text-[9px] font-black uppercase tracking-[0.2em] border transition-all duration-300 ${
                activeFilter === key 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-neutral-900 border border-neutral-900">
          {(filteredItems as ReportItem[]).map((report) => (
            <div 
              ...
              className="bg-[#050505] p-10 flex flex-col justify-between group hover:bg-neutral-950 transition-colors duration-500 min-h-[320px] cursor-pointer"
            >
              <div className="space-y-6">
                <div className="flex justify-between items-start">
                  <span className="text-[8px] font-black font-mono text-neutral-700 tracking-[0.5em]">ID_{report.id}</span>
                  <div className="w-2 h-2 rounded-full bg-[#58B573] opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
                <h3 className="text-2xl font-black uppercase tracking-tight group-hover:text-[#FFEE00] transition-colors">
                  {report.title}
                </h3>
                <p className="text-neutral-500 text-sm leading-relaxed uppercase tracking-wider font-medium">
                  {report.desc}
                </p>
              </div>
              
              <div className="pt-8 flex justify-between items-center">
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-neutral-800">
                  {t.categories[report.cat as keyof typeof t.categories]}
                </span>
                <button className="text-[9px] font-black uppercase tracking-[0.4em] border-b border-white pb-1 opacity-0 group-hover:opacity-100 transition-all">
                  {t.ui.access}
                </button>
              </div>
            </div>
        ...
        {/* Intelligence Footer */}
        <div className="mt-24 pt-24 border-t border-neutral-900 flex flex-col md:flex-row justify-between items-end gap-12">
           ...
           <div className="text-right">
              <span className="text-[8px] font-mono text-neutral-800 block mb-2 uppercase">Systems Status: ACTIVE</span>
              <span className="text-[8px] font-mono text-neutral-800 block uppercase">Last update: 2026_Q1_SYD</span>
           </div>
        </div>
      </main>
